...
### リンクを開いたのにログイン画面に戻される

まず **URL に `error=` が付いているか**を見ます。`/auth/callback` は失敗すると必ず
`?error=` を付けて返すので、これが切り分けの分岐点になります。

| URL | 意味 |
| --- | --- |
| `/login?error=otp_expired` / `access_denied` | Supabase 側が期限切れ・使用済みと判定した。送り直す |
| `/login?error=exchange_failed` | `/auth/callback` までは来た。セッションの確立に失敗した。**メールを複数送っていて古い方を開いた**か、**メールを送ったブラウザと違うブラウザで開いた**か。前者の方が起きやすい（下記） |
| `/login?error=missing_code` | リンクにログイン情報が入っていなかった |
| `error=` が無い | **`/auth/callback` を通っていない**。コードが別の場所に着地している |

`exchange_failed` で「古いメール」が先に来る理由: この版の auth-js は PKCE の検証用の
値をフローごとに持ちつつ、ルートハンドラから読める固定キーには**最後に送ったぶんだけ**を
書く（`auth-js/src/lib/helpers.ts`）。**2 通送って古い方のリンクを開くと、同じブラウザでも
失敗する。**

いずれの場合も、画面に理由と次の手が出ます（`describeCallbackError`）。

最後の「`error=` が無い」は、Site URL への差し替えでコードがサイトのルートに
落ちた場合です。**いまは `proxy.ts` が、自分のドメインのどこに着地した
ログイン用の値でも `/auth/callback` に転送する**ので、この経路でも成立します。
判定は `lib/auth-callback-params.ts`。

`exchange_failed` は、ログイン画面に出る貼り付け欄で回避できます。ただし
**メール本文のリンク**（`supabase.co` で始まるもの）を貼ったときだけです。あちらは
`verifyOtp` で token を直接検証し、PKCE の検証用の値を使わないため、ブラウザが違っても
古いメールでも通ります。リンクを開いた先のアドレス（`?code=…`）を貼った場合は
`exchangeCodeForSession` になるので、同じ制限がかかります。

### なぜ `token_hash` を GET のコールバックで受けないか

`verifyOtp({token_hash, type})` は**メールアドレスにも PKCE の検証用の値にも
紐付きません**。これをどのパスでも自動で拾うと、攻撃者が自分宛に取った token を
仕込んだ URL を 1 回踏ませるだけで、**被害者を攻撃者のアカウントにログインさせられます**
（既存のセッションも差し替わる）。記録は攻撃者の行に入り、画面上それと分かる表示は
ありません。

そのため `proxy.ts` と `/auth/callback` が拾うのは `code` だけです。`code` の交換には
送信したブラウザにしか無い検証用の値が要るので、他人の `code` を踏ませても成立しません。
`token_hash` は**貼り付けフォームにだけ**残してあります。利用者が意図して貼る操作が
要るので、1 クリックの経路にはなりません。

### メールテンプレートが編集できないとき

6 桁コードをメールに入れるには Magic Link テンプレートに `{{ .Token }}` が要りますが、
**Supabase はメールテンプレートの編集を独自 SMTP の設定とセットにしています**。
SMTP を用意しないとテンプレートの編集欄はロックされたままで、本文にコードは入りません。

そのため、ログイン画面には**リンクを貼り付けて入る**道を用意しています。
リンクの戻り先が壊れていても、リンクそのものには token が入っているので、
戻り先を経由せずアプリ側で検証すれば入れます。

1. メール本文のボタンを長押し（PC なら右クリック）してリンクのアドレスをコピーする
2. ログイン画面の「コードが本文に無いとき」に貼り付ける

`https://<ref>.supabase.co/auth/v1/verify?token=…` のほか、リンクを開いた結果
アドレスバーに出た `…/auth/callback?code=…` も受け付けます。読み取りは
`lib/auth-link.ts`、検証は `verifyMagicLinkUrl`（`app/login/actions.ts`）にあります。

ただしこれは**リンクが壊れている状態を回避する手段**であって、直したことにはなりません。
Site URL と Redirect URLs は上のとおり直してください。
