...
| URL | 意味 |
| --- | --- |
| `/login?error=exchange_failed` | `/auth/callback` までは来た。セッションの確立に失敗した。ほぼ**メールを送ったブラウザとリンクを開いたブラウザが違う**（PKCE の検証用の値は送信したブラウザにしか無い）。メールアプリ内のブラウザで開くと起きる |
...
`exchange_failed` は、ログイン画面に出る貼り付け欄で回避できます。貼り付けは
リンクの token を直接検証し、PKCE の検証用の値を使わないため、ブラウザが違っても
通ります。
