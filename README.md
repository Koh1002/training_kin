...
メールアドレスとパスワード。初回は「新規登録」でアカウントを作ります。

以前はマジックリンクでしたが、**アプリ側では直せない理由で通りませんでした**。
リンクの戻り先は正しく届いていたのに、PKCE のコード交換に必要な値がメールを送った
ブラウザの Cookie にしか無く、スマートフォンでメールアプリ内のブラウザがリンクを
開くと必ず失敗します。`@supabase/ssr` は `flowType` を `pkce` に固定していて
上書きできず（`ssr/src/createServerClient.ts`）、6 桁コードに逃げる道も、Supabase が
メールテンプレートの編集を独自 SMTP とセットにしているため塞がっていました。

メールの往復をやめると、戻り先の設定・PKCE・送信上限（1時間2通）がまとめて要らなく
なります。利用者が 1 人なので、この交換で失うものはほぼありません。
...
**右下の鉛筆ボタン**から記録します。どの画面からでも同じ場所で、筋トレも英語も。

以前は入力欄がページのいちばん下にあり、人体図が縦に長いぶんスクロールしないと
届きませんでした。見せるためのものが、使うためのものの前に立っていました。

### 声で記録する（筋トレ）

「ベンチプレス10キロ10回3セット、スクワット20キロ10回3セット」のように続けて
言えます。Web Speech API を使うので、サーバも API キーも要りません。

**言われたとおりには保存しません。** 音声認識は必ず聞き間違えるので、

1. 発話を種目ごとに割る（`lib/workout/voice-parse.ts`）
2. 種目名をマスタに寄せる（`lib/workout/match-exercise.ts`）
3. **確認画面で直してから保存する**

の 3 段を通します。寄せは `name_ja` と `memo_alias` の両方を見る文字バイグラムの
Dice 係数で、「肩前ダンベル」→「フロントレイズ（ダンベル）」のように自分の呼び方から
辿れます。**確信が持てないときは当てません**——一番近いものを無理に選ぶと、確認画面で
正しく見えてしまい、違う種目のまま保存されるためです。

対応していないブラウザでは、同じ書き方を打ち込む欄が出ます。音声認識には HTTPS と
マイクの許可が要ります。
...
3. Authentication > Providers で **Email** を有効にし、**「Confirm email」を切る**。

   これが有効だと、新規登録のときに Supabase が確認メールを送り、**セッションを
   返しません**。登録できたように見えて入れない、という一番分かりにくい形になります
   （アプリはこれを検出して画面に理由を出しますが、切るのが本筋です）。

4. すでにマジックリンクでアカウントを作っていた場合は、Authentication > Users で
   **そのユーザーを削除する**。

   パスワードの無いアカウントに、後からアプリ側でパスワードを付けることはできません。
   `signUp` は既存のアドレスに対して、アカウントの存在を漏らさないためセッションを
   返さないからです。ログインできたことが無ければデータは入っていません。

5. アプリの画面から「新規登録」でアカウントを作る。

6. 作り終えたら、Authentication > Providers > Email の
   **「Allow new users to sign up」を切る**。

   1 人用なので、以後は誰も登録できないようにしておきます。切らないと、ログイン画面に
   到達した人が任意のアドレスでアカウントを作れます。

**SMTP の設定も、URL Configuration も要りません。** メールを 1 通も送らないためです。
...
| Email プロバイダが有効か | ログインも新規登録もできない |

**「Confirm email」と「Allow new users to sign up」は API から確認できない**ので、
この診断の対象外です。上のセットアップ手順に沿って手で確認してください。
...
2. 環境変数を設定する。
   ...
   **Vercel の環境変数は Production / Preview / Development で別々に管理されます。**
   Production にだけ登録すると、ブランチ名が入ったプレビュー URL では未設定のままです。
   その状態では画面に「環境変数が設定されていません」と不足している変数名が出ます
   （`proxy.ts`）。
