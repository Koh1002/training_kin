...
export function describeCallbackError(code: string | undefined): string | null {
  switch (code) {
    ...
    case 'exchange_failed':
      // ほとんどの場合、メールを送ったブラウザとリンクを開いたブラウザが違う。
      // ログインの途中の値は送信したブラウザにしか無いため成立しない。
      // メールアプリ内のブラウザで開くと起きやすい。
      return (
        'リンクは受け取りましたが、ログインを完了できませんでした。' +
        'メールを送ったブラウザとは別のブラウザで開いた可能性があります。' +
        '同じブラウザで開き直すか、下の「コードが本文に無いとき」にリンクを貼り付けてください。'
      )
    default:
      return null
  }
